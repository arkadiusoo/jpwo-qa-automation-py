...
def test_post_brand_happy(http, base_url, timeout, admin_headers, brand_factory, brand_payload_valid):
    bid = brand_factory(brand_payload_valid())
    r = http.get(f"{base_url}/brands/{bid}", headers=admin_headers, timeout=timeout)
    assert r.status_code == 200, f"Spec expects 200, got {r.status_code}: {r.text}"
